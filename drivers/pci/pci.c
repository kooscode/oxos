...
void BootDetectMemorySize(void)
{
  ...
  //force to 128mb to try mem tests
  PciWriteDword(BUS_0, DEV_0, FUNC_0, 0x84, 0x7FFFFFF);  // 128 MB

  //fill top 128mb bas with data        
  ...
  {
      // maybe a 128MB box  - lets double check by filling with
      // different data and validating only high mem was set..
      memset(fillstring,0x55,0x200);
      memset(membasetop,0x55,0x200);
      asm volatile ("wbinvd\n");
      
      //check only high mem was affected..
      if ((memcmp(membasetop,fillstring,0x200) == 0)  && 
          (memcmp(membaselow,fillstring,0x200) != 0))
      {
        xbox_ram = 128;
      }
  }

  // 128mb not found. reset back to 64
  if (xbox_ram == 64) 
