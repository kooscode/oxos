...
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Divide by Zero");
}
void IntHandlerException1C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Single Step");    while(1) ;
}

void IntHandlerException2C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: NMI");    while(1) ;
}
void IntHandlerException3C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Breakpoint");    while(1) ;
}
void IntHandlerException4C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Overflow Trap");    while(1) ;
}
void IntHandlerException5C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: BOUND exceeded");    while(1) ;
}
void IntHandlerException6C(void)
{
    unsigned int dwEbp = 0;
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Invalid Opcode");
    __asm__ __volatile__ ( " mov %%esp, %%eax\n " : "=a" (dwEbp) );
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "   %08X:%08X", *((volatile unsigned int *)(dwEbp+0x48)), *((volatile unsigned int *)(dwEbp+0x44)));
    while(1) ;
}
...
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Coprocessor Absent");
}
void IntHandlerException8C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Double Fault");    while(1) ;
}
//void IntHandlerException9C(void) {    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Copro Seg Overrun\n");    while(1) ; }
void IntHandlerException9C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Copro Seg Overrun");
}
void IntHandlerExceptionAC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Invalid TSS");    while(1) ;
}
void IntHandlerExceptionBC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Segment not present");    while(1) ;
}
void IntHandlerExceptionCC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Stack Exception");    while(1) ;
}
void IntHandlerExceptionDC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: General Protection Fault");    while(1) ;
}
void IntHandlerExceptionEC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Page Fault");    while(1) ;
}
void IntHandlerExceptionFC(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Reserved");    while(1) ;
}
//void IntHandlerException10C(void) {    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Copro Error\n");    while(1) ; }
void IntHandlerException10C(void)
{
    BootLogger(DEBUG_EXCEPTIONS, DBG_LVL_FATAL, "CPU Exc: Copro Error");
}
